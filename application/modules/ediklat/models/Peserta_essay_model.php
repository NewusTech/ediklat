...
class Peserta_essay_model extends CI_Model
{

	var $table = 'essay_member';
	var $column_order = array('member.name', 'member.agency'); //set column field database for datatable orderable
	var $column_search = array('member.name', 'member.agency'); //set column field database for datatable searchable just firstname , lastname , address are searchable
	...
	private function _get_datatables_query($essayCode = '')
	{
		...
		->where('essayCode', $essayCode)->where('essay_member.deleteAt', NULL);


		if($this->input->post('state_participant') != NULL ){
			$this->db->where('member.stateCode', $this->input->post('state_participant'));
		}

		if($this->input->post('education_service_participant') != NULL ){
			$this->db->where('member.education_service', $this->input->post('education_service_participant'));
		}
		$i = 0;
