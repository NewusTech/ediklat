<?php if ($answer == NULL) : ?>
    <p class="text-bold text-sm text-center">Anda belum mengisi jawaban</p>
<?php endif; ?>
<?php if ($answer != NULL) : ?>
    <?php foreach ($answer as $k => $v) : ?>
        <div class="d-flex gap-2 align-items-center justify-content-start">
            <?php if ($kegiatan->status == 'open') : ?>
                <a href="javascript:void(0)" onclick="hapusJawaban(<?php echo $v['ptCode'] ?>)" class="text-danger" title="Hapus" style="font-family: 'Nunito', sans-serif;"><i class="fa fa-trash"></i></a>
            <?php endif; ?>
